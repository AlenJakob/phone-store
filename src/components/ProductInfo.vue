<template  >
  <div>
    <article
      ...
      v-for="product in getProductInfo"
      :key="product.id"
    >
      <img
        :src="product.image"
      ...
      <div class="min-w-0 relative flex-auto sm:pr-20 lg:pr-0 xl:pr-20">
        ...
        <dl class="flex flex-wrap text-sm font-medium whitespace-pre">
          <div>
            ...
            <dd>
              <abbr :title="`${product.name} minutes`">a{{}}m</abbr>
            </dd>
          </div>
          <div>
          ...
          <div class="flex-none w-full mt-0.5 font-normal">
            ...
            <dd class="inline text-black">aff{{}}</dd>
          </div>
          <div
...
export default {
  ...
  data() {
    return {
      productInfo: this.$store.state.products,
    };
  },
  methods: {
    getProductInfo() {
      return this.$store.state.products.map((element) => {
        console.log("***");
        console.log(element);
      });
    },
